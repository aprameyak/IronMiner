...
import type { TimelineEntry, VideoJob } from '../types'

const LOCAL_KEY = (siteId: string): string => `ironsite_timeline_${siteId}`
...
export default function BriefingView({ text, siteId, usingMock, onJobComplete }: BriefingViewProps) {
  const [localEntries, setLocalEntries] = useState<TimelineEntry[]>([])
  const [apiEntries, setApiEntries]     = useState<TimelineEntry[]>([])
  ...
  useEffect(() => {
    ...
    try {
      ...
      setLocalEntries(raw ? (JSON.parse(raw) as TimelineEntry[]) : [])
  ...
  useEffect(() => {
    ...
    fetchTimeline(siteId)
      .then(data => setApiEntries(Array.isArray(data) ? (data as TimelineEntry[]) : []))
      .catch(() => setApiEntries([]))
  }, [siteId, usingMock])
  ...
  const refreshJobs = useCallback(() => {
    ...
    fetchJobs(siteId)
      ...
      .catch(() => setJobs([]))
  }, [siteId, usingMock, onJobComplete])
  ...
  }, [jobs, refreshJobs, usingMock])

  // ── Persist local entry ───────────────────────────────────────────────────
  const pushLocal = (entry: TimelineEntry) => {
  ...
  const mockEntries: TimelineEntry[] = usingMock ? (MOCK_TIMELINE[siteId] || []) : []
  const allEntries = [
    ...localEntries,
    ...
    <div>
      ...
      <div style={{ marginBottom: 28 }}>
        ...
        <div style={{ display: 'flex', gap: 10, marginBottom: 10, alignItems: 'center' }}>
          <input
            value={noteWho}
            onChange={(e: React.ChangeEvent<HTMLInputElement>) => setNoteWho(e.target.value)}
          ...
          <div style={{ flex: 1, display: 'flex', gap: 8 }}>
            <input
              ...
              onKeyDown={(e: React.KeyboardEvent) => { if (e.key === 'Enter') handleLog() }}
