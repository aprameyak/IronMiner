...
const SOURCE_COLOR: Record<string, { dot: string; bg: string; border: string; text: string }> = {
...
function AlertMode({ alert, expanded, onToggle }: AlertModeProps) {
...
function TimelineMode({ entry, expanded, onToggle }: TimelineModeProps) {
...
export default function AlertCard({ mode = 'alert', alert, entry, expanded, onToggle }: AlertCardProps) {
  if (mode === 'timeline' && entry) {
