...
interface WorkerVideoGridProps {
  workerStreams: Map<string, WorkerStream>
  selectedIdentity: string | null
  onSelectWorker: (identity: string) => void
}

export default function WorkerVideoGrid({ workerStreams, selectedIdentity, onSelectWorker }: WorkerVideoGridProps) {
  ...
  if (count === 0) {
    return (
      <div style={{
        ...
        background: C.surface, borderRadius: 12, border: `1px dashed ${C.border}`, aspectRatio: '16/9',
      }}>
  ...
  return (
    <div style={{ display: 'grid', gridTemplateColumns: count === 1 ? '1fr' : '1fr 1fr', gap: 8 }}>
