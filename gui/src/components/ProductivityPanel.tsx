...
const trendColor: Record<string, { bg: string; border: string; text: string }> = {
...
const congestionBar = (score: number): string => {
...
function parseSummary(raw: string | null | undefined): string[] {
  if (!raw) return []
  let text = raw.trim()
  if (text.startsWith('{')) {
    try {
      const parsed = JSON.parse(text) as Record<string, unknown>
      text = (parsed['summary'] ?? parsed['text'] ?? Object.values(parsed)[0] ?? text) as string
    } catch {
...
export default function ProductivityPanel({ siteId, hasFootage = true }: ProductivityPanelProps) {
  const [view, setView] = useState<'overview' | 'benchmarks'>('overview')
  const [report, setReport] = useState<ProductivityReport | null>(null)
  const [loading, setLoading] = useState(true)
  const [expandedZone, setExpandedZone] = useState<number | null>(null)
  const [expandedOverlap, setExpandedOverlap] = useState<number | null>(null)
  ...
  useEffect(() => {
    ...
    try {
      ...
      ws.onmessage = (e) => {
        try {
          const msg = JSON.parse(e.data as string) as { stage?: string }
      ...
      {report.trade_overlaps?.length > 0 && (
        <div style={{ marginBottom: 24 }}>
          ...
          {report.trade_overlaps.map((o, i) => {
            const sty = severityBadge[(o as any).severity] ?? severityBadge['minor']
