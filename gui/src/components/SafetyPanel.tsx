...
const riskColor: Record<string, { bg: string; border: string; text: string }> = {
...
export default function SafetyPanel({ siteId }: SafetyPanelProps) {
  ...
  const handleRun = async () => {
    ...
    setDismissed({})
    try {
      let videoJobId = 'mock_vj_001'
      try {
        const jobs = await fetchJobs(siteId)
        const completed = (Array.isArray(jobs) ? jobs : [])
          .filter(j => j.status === 'completed')
          .sort((a, b) => new Date(b.created_at).getTime() - new Date(a.created_at).getTime())
        if (completed.length > 0) videoJobId = completed[0].job_id
      } catch {}
      await runSafetyAnalysis(siteId, videoJobId)
      const data = await fetchSafetyReport(siteId)
      setReport(data)
    } catch {
