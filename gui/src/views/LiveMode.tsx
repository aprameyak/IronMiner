...
import type { Site, SiteWorker, Team, FeedConfig } from '../types'
...
function getLiveIdentityForWorker(worker: SiteWorker, liveWorkerStreams: Map<string, WorkerStream>): string | null {
...
function WorkerRow({ worker, isLive, selected, onClick }: WorkerRowProps) {
  ...
  return (
    <button
      ...
      onMouseOver={e => { if (!selected) (e.currentTarget as HTMLButtonElement).style.background = 'rgba(255,255,255,0.03)' }}
    ...
    >
      ...
      {isLive ? (
        <span style={{ fontSize: 9, fontFamily: 'var(--mono)', fontWeight: 700, color: '#22C55E', letterSpacing: '0.1em' }}>
          LIVE
        </span>
      ) : (
        <span style={{ fontSize: 9, fontFamily: 'var(--mono)', color: `${tradeColor}99`, letterSpacing: '0.02em' }}>
          {worker.trade}
        </span>
      )}
    </button>
...
interface TeamSectionProps {
  team: Team
  workers: SiteWorker[]
  liveWorkerStreams: Map<string, WorkerStream>
  selectedWorkerIdentity: string | null
  onSelectWorker: (identity: string | null) => void
}

function TeamSection({ team, workers, liveWorkerStreams, selectedWorkerIdentity, onSelectWorker }: TeamSectionProps) {
  ...
  return (
    <div style={{ marginBottom: 18 }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 4, paddingLeft: 2 }}>
        ...
        <span style={{ fontSize: 10, fontWeight: 600, fontFamily: 'var(--mono)', color: '#64748b', letterSpacing: '0.08em', textTransform: 'uppercase', flex: 1 }}>
          {team.name}
        </span>
        {liveCount > 0 && (
          <span style={{ fontSize: 9, fontFamily: 'var(--mono)', color: '#22C55E', background: 'rgba(34,197,94,0.1)', padding: '1px 6px', borderRadius: 3 }}>
            {liveCount} live
          </span>
        )}
      </div>
      {team.task && (
        <div style={{ fontSize: 10, color: '#334155', paddingLeft: 11, marginBottom: 6, fontStyle: 'italic', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
          {team.task}
        </div>
      )}
      {workers.length === 0 ? (
...
export default function LiveMode() {
  ...
  const [liveWorkerStreams, setLiveWorkerStreams] = useState<Map<string, WorkerStream>>(new Map())
  const [selectedWorkerIdentity, setSelectedWorkerIdentity] = useState<string | null>(null)
  ...
  useEffect(() => {
    fetchFeeds()
      .then((data: FeedConfig[]) => { setFeeds(data); if (data.length && !selectedFeed) setSelectedFeed(data[0].id) })
      .catch(() => { setUsingMock(true); setFeeds(MOCK_FEEDS); setSelectedFeed(MOCK_FEEDS[0].id) })

    fetchSites()
      .then((data: Site[]) => { setSites(data); if (data.length) setSelectedSite(data[0].id) })
  ...
  return (
    <div style={{ display: 'grid', gridTemplateColumns: '1fr 280px', gap: 20 }}>
        ...
        {livekitAvailable ? (
          ...
          <>
            <div style={{
              ...
              background: '#080B10', border: '1px solid rgba(255,255,255,0.06)',
              ...
              <div style={{ position: 'absolute', inset: 0, display: 'flex', alignItems: 'center', justifyContent: 'center', flexDirection: 'column', gap: 8 }}>
                <div style={{ width: 40, height: 40, borderRadius: '50%', border: '1px solid rgba(255,255,255,0.07)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
              ...
              <div style={{ position: 'absolute', top: 12, right: 14 }}>
                <span style={{ fontSize: 9, fontFamily: 'var(--mono)', color: '#334155', background: 'rgba(0,0,0,0.4)', padding: '3px 8px', borderRadius: 4, letterSpacing: '0.06em' }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 10, marginBottom: 12 }}>
              <button disabled title="Connect to enable" style={{ width: 44, height: 44, borderRadius: 10, border: '1px solid rgba(255,255,255,0.08)', background: 'rgba(255,255,255,0.04)', cursor: 'not-allowed', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#475569', opacity: 0.6 }}>
                <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
                  <path d="M12 1a3 3 0 0 0-3 3v8a3 3 0 0 0 6 0V4a3 3 0 0 0-3-3z" />
                  <path d="M19 10v2a7 7 0 0 1-14 0v-2M12 19v4M8 23h8" />
                </svg>
              </button>
              <button title="Snapshot" style={{ width: 44, height: 44, borderRadius: 10, border: '1px solid rgba(255,255,255,0.08)', background: 'rgba(255,255,255,0.06)', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#94a3b8' }}>
                <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
                  <path d="M23 19a2 2 0 0 1-2 2H3a2 2 0 0 1-2-2V8a2 2 0 0 1 2-2h4l2-3h6l2 3h4a2 2 0 0 1 2 2z" />
                  <circle cx="12" cy="13" r="4" />
                </svg>
              </button>
              <button title="Flag issue" style={{ width: 44, height: 44, borderRadius: 10, border: '1px solid rgba(239,68,68,0.25)', background: 'rgba(239,68,68,0.12)', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#FCA5A5' }}>
                <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
      ...
      <div style={{ display: 'flex', flexDirection: 'column', minHeight: 0 }}>
        <div style={{ display: 'flex', alignItems: 'center', borderBottom: '1px solid rgba(255,255,255,0.06)', marginBottom: 14 }}>
          {([['all', `All · ${siteWorkers.length}`], ['teams', 'Teams']] as [string, string][]).map(([id, label]) => {
            const active = tab === id
            return (
              <button
                key={id}
              ...
              >
                {label}
                {active && (
                  <span style={{ position: 'absolute', bottom: 0, left: '50%', transform: 'translateX(-50%)', width: 'calc(100% - 16px)', maxWidth: 48, height: 2, background: '#D97706', borderRadius: 1 }} />
                )}
              </button>
            )
          })}
          <select
            ...
            style={{ marginLeft: 'auto', border: '1px solid rgba(255,255,255,0.08)', borderRadius: 6, padding: '6px 8px', cursor: 'pointer', fontSize: 11, fontFamily: 'inherit', maxWidth: 120 }}
          >
        ...
        {liveWorkerStreams.size > 0 && (
          <div style={{ marginBottom: 18 }}>
            <div style={{ fontSize: 9, fontFamily: 'var(--mono)', color: '#22C55E', letterSpacing: '0.12em', marginBottom: 8 }}>
        ...
        <div style={{ overflowY: 'auto', flex: 1, maxHeight: 'calc(100vh - 220px)' }}>
          {tab === 'all' ? (
            ...
            siteTeams.length === 0 ? (
              <div style={{ fontSize: 12, color: '#334155', textAlign: 'center', padding: '48px 0' }}>No teams planned today</div>
            ) : (
              <>
                {siteTeams.map(team => (
                  <TeamSection
                    key={team.id}
                    team={team}
                    workers={team.worker_ids.map(id => siteWorkers.find(w => w.id === id)).filter((w): w is SiteWorker => Boolean(w))}
                ...
                {unassignedWorkers.length > 0 && (
                  <div style={{ marginBottom: 18 }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 4, paddingLeft: 2 }}>
                      ...
                      <span style={{ fontSize: 10, fontWeight: 600, fontFamily: 'var(--mono)', color: '#334155', letterSpacing: '0.08em', textTransform: 'uppercase' }}>
                        Unassigned
                      </span>
                    </div>
