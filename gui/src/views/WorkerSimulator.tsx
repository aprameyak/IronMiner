...
import type { Site, SiteWorker } from '../types'
...
export default function WorkerSimulator() {
  ...
  const managerAudioRef = useRef<HTMLAudioElement>(null)
  const roomRef = useRef<Room | null>(null)
  const localVideoTrackRef = useRef<ReturnType<typeof createLocalVideoTrack> extends Promise<infer T> ? T : never | null>(null as any)

  useEffect(() => {
    fetchSites()
      .then((data: Site[]) => { setSites(data); if (data.length) setSiteId(data[0].id) })
      .catch(() => { setSites(MOCK_SITES); setSiteId(MOCK_SITES[0].id) })
  }, [])
  ...
  useEffect(() => {
    ...
    fetchSiteWorkers(siteId)
      .then((workers: SiteWorker[]) => { setSiteWorkers(workers); if (workers.length) setSelectedWorkerId(workers[0].id) })
      .catch(() => {
  ...
  const byTrade = siteWorkers.reduce<Record<string, SiteWorker[]>>((acc, w) => {
  ...
  const connect = async () => {
    ...
    try {
      ...
      localVideoTrackRef.current = null
      setStatus('error')
      setError((err as Error).message || 'Failed to connect. Allow camera and microphone when prompted.')
    }
  }
  ...
  useEffect(() => {
    ...
    const attachAudio = (track: any) => {
      if (track.kind === Track.Kind.Audio) { track.attach(el); el.play().catch(() => {}) }
    }

    const onTrackSubscribed = (track: any, _pub: any, participant: any) => {
      ...
      <div style={{ minHeight: '100vh', background: '#090C12', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 24, fontFamily: 'var(--body)', color: '#E2E8F0' }}>
        <div style={{ width: '100%', maxWidth: 440 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 20 }}>
            ...
            <span style={{ fontSize: 10, fontWeight: 700, fontFamily: 'var(--mono)', color: tradeColor, background: `${tradeColor}20`, padding: '3px 8px', borderRadius: 4, letterSpacing: '0.05em' }}>
      ...
      <div style={{ width: '100%', maxWidth: 400 }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
          ...
          <div>
            ...
            <select value={selectedWorkerId} onChange={e => setSelectedWorkerId(e.target.value)} style={inputStyle} disabled={siteWorkers.length === 0}>
