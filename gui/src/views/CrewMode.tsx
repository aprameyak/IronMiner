...
import type { Site, SiteWorker, Team } from '../types'
...
interface Assignment { team_name: string; zone: string; task: string }
  ...
  return (
    <div>
      <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 24, flexWrap: 'wrap' }}>
        <select value={selectedSite || ''} onChange={e => setSelectedSite(e.target.value)} style={{ ...selectStyle, fontSize: 15, fontWeight: 600, color: '#f1f5f9' }}>
      ...
      {visible.length === 0 ? (
        <div style={{ textAlign: 'center', padding: '80px 0', border: '1.5px dashed rgba(255,255,255,0.05)', borderRadius: 12, color: '#475569', fontSize: 14 }}>
          No workers for this site
        </div>
      ) : (
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(240px, 1fr))', gap: 14 }}>
          {visible.map(w => (
            <WorkerSummaryCard key={w.id} worker={w} assignment={assignmentMap[w.id] || null} onClick={() => setSelectedWorker(w)} tradeColors={TRADE_COLORS} />
          ))}
        </div>
      )}
...
interface WorkerSummaryCardProps {
  ...
  onClick: () => void
  tradeColors: Record<string, { bg: string; text: string }>
}

function WorkerSummaryCard({ worker, assignment, onClick, tradeColors }: WorkerSummaryCardProps) {
  const tradeColor = tradeColors[worker.trade] || { bg: 'rgba(255,255,255,0.08)', text: '#94a3b8' }
  return (
    <button
      onClick={onClick}
      style={{ all: 'unset', display: 'block', width: '100%', boxSizing: 'border-box', padding: '16px 18px', borderRadius: 12, background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,255,255,0.08)', cursor: 'pointer', textAlign: 'left', minHeight: 96, transition: 'border-color 0.15s, background 0.15s' }}
      onMouseOver={e => { (e.currentTarget as HTMLButtonElement).style.borderColor = 'rgba(249,115,22,0.35)'; (e.currentTarget as HTMLButtonElement).style.background = 'rgba(249,115,22,0.04)' }}
      onMouseOut={e => { (e.currentTarget as HTMLButtonElement).style.borderColor = 'rgba(255,255,255,0.08)'; (e.currentTarget as HTMLButtonElement).style.background = 'rgba(255,255,255,0.04)' }}
    >
      <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 8 }}>
        <div style={{ width: 8, height: 8, borderRadius: '50%', flexShrink: 0, background: tradeColor.text, boxShadow: `0 0 6px ${tradeColor.text}88` }} />
        <div style={{ fontSize: 15, fontWeight: 700, color: '#f1f5f9', flex: 1 }}>{worker.name}</div>
      </div>
      <div style={{ display: 'inline-block', marginBottom: 10, fontSize: 10, fontWeight: 500, padding: '2px 7px', borderRadius: 4, background: tradeColor.bg, color: tradeColor.text, fontFamily: 'var(--mono)', letterSpacing: '0.04em' }}>
        {worker.trade}
      </div>
      {assignment ? (
        <div>
          <div style={{ fontSize: 12, color: '#94a3b8', marginBottom: 2 }}>{assignment.team_name}</div>
          {assignment.zone && <div style={{ fontSize: 11, color: '#475569', fontFamily: 'var(--mono)', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{assignment.zone.split('—')[0].trim()}</div>}
        </div>
      ) : (
